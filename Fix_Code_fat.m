...
for i = 1:4
    rmsPlots(i) = plot(nan, nan, 'Color', colors{i}, 'LineWidth', 1.5, 'Marker', 'o', 'MarkerSize', 4);
end
...
mpfPlots = gobjects(4);
for i = 1:4
    mpfPlots(i) = plot(nan, nan, 'Color', colors{i}, 'LineWidth', 1.5, 'Marker', 's', 'MarkerSize', 4);
end
...
while true
        ...
        if s.NumBytesAvailable > 0
            try
                ...
                if length(dataStr) == numChannels
                    numericData = str2double(dataStr);
                    if all(isfinite(numericData))  % Only accept valid data (without NaN)
                        sampleCount = sampleCount + 1;
                        dataBuffer(sampleCount, :) = numericData;
                    end
                end
            catch
                continue;
    ...
    for i = 1:4
        ...
        if all(isfinite(signal))  % Double check for safety
            [pxx, f] = pwelch(signal, hann(256), 128, [], Fs);
